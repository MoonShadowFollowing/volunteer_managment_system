...
> 数据库系统课程设计
> **前端（Vue3）+ 后端（Spring Boot）+ 文档** 全套，clone 一个分支即可运行整个项目。
...
`202400010101`（组织者）发活动 → `12345678`（管理员）审核通过 → `202400010101` 开「发布中」 → `202400020101`（志愿者）报名 →
`202400010101` 审核报名 → `202400010101` 改志愿时/补签 → `202400020101` 我的证书出现并可下载 PDF →
把工时改 0 → 证书自动失效。
